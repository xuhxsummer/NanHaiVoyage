...
public final class VoyageHud extends Group implements Disposable {
    ...
    public VoyageHud(Skin skin,Runnable captain,IntConsumer stat,Runnable[] shortcuts,Runnable world,
        ...
        for(int i=1;i<4;i++){final int idx=i;Table cell=new Table();cell.setName(names[i]);cell.add(icon(icons[i])).size(40).padRight(4);
        ...
        toastBar.add(noticeHead).growX().height(36).padBottom(8).row();
        toast=new MarqueeNotice(ui.label("",24,Color.valueOf("3F3326")));
        toastBar.add(toast).grow();toastBar.setTouchable(Touchable.disabled);
    ...
    public void layoutViewport(float width,float height) {
        ...
        toastBar.setX((getWidth()-toastBar.getWidth())/2);
    }
    ...
    public void update(GameState g,boolean neutral,boolean ready,float kx,float ky){
        ...
        // 这里只刷新文字。
        boolean island=p<0 && isle>=0;
        location.setName(island?"探索":"所在地");
        placeSub.setVisible(!island);location.getCell(placeSub).height(island?0:20);
        if(p>=0){place.setText(Catalog.PORTS[p]+"港");placeSub.setText(g.dockedPort>=0?"已停泊 · 点击经营":"点击停靠 · 港口经营");}
        else if(isle>=0){place.setText("探索");placeSub.setText("");}
    }

...
public final class VoyageMinimap extends Actor implements Disposable {
    ...
    @Override public void draw(Batch batch,float alpha){
        ...
        for(int i=0;i<Catalog.PORTS.length+Catalog.ISLANDS.length;i++) {
            ...
            float dx=((port?Catalog.PORT_X[id]:Catalog.ISLAND_X[id])-wx)*scale;
            float dy=((port?Catalog.PORT_Y[id]:Catalog.ISLAND_Y[id])-wy)*scale;
            if(dx*dx+dy*dy<(r-21)*(r-21)) {
                Drawable art=port?IconLib.port(id):IconLib.island(id);
